...
1.   互斥性
     ...
     在任意时刻，只能有一个客户端可以持有锁，使用nx参数保证互斥性，只有一个线程可以设置成功，其它线程均设置失败
...
4.   加锁解锁必须是同一个线程
     ...
     加锁时加入线程ID和服务器实例标识，释放锁时先判断是否为当前服务器实例的线程加的锁，然后再释放锁，避免被其他线程误删而提前释放锁。

5.   容错性

     配置Redis集群，只要超过半数的Redis加锁成功就代表加锁成功，防止一个Redis实例宕机导致加锁失败
